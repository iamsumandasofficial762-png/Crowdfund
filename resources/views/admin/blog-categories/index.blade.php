...
<head>
    ...
    <style>
        ...
        .status-badge.active { color:#087a2a; background:#e0f7e8; }
        .status-badge.inactive { color:#9b2417; background:#fde3e0; }
    ...
    <div class="admin-layout">
        <aside class="sidebar">
            ...
            <nav>
                ...
                <a class="nav-link active" href="{{ route('admin.blog-categories.index') }}"><i class="fa-solid fa-tags"></i> Blog Categories</a>
        ...
        <main>
            <header class="topbar">
                <div class="topbar-inner d-flex align-items-center justify-content-between gap-3">
                    <div>
                        <p class="text-muted small mb-1">Story taxonomy</p>
                        <h1 class="h4 fw-bold mb-0">Blog Categories</h1>
                    </div>
                    <div class="topbar-actions">
                        ...
                        <div class="dropdown">
                            <button class="btn profile-btn dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="fa-solid fa-user-circle text-warning"></i>{{ auth()->user()->name ?? 'Admin' }}
                            </button>
                            <ul class="dropdown-menu dropdown-menu-end"><li><a class="dropdown-item" href="{{ route('admin.settings.index') }}">Profile</a></li></ul>
                        </div>
            ...
            <div class="content">
                ...
                @endif
                @if ($errors->any())
                    <div class="alert alert-danger fw-bold">Please fix the highlighted fields.</div>
                @endif

                <div class="row g-4">
                    <div class="col-lg-4">
                        <form class="panel p-4" action="{{ route('admin.blog-categories.store') }}" method="post">
                            @csrf
                            <h2 class="h5 fw-bold mb-3">Create Category</h2>
                            <div class="mb-3">
                            ...
                            <button class="btn btn-warning fw-bold" type="submit">Create Category</button>
                        </form>
                    </div>
                    <div class="col-lg-8">
                                ...
                                <tbody>
                                    @forelse ($categories as $category)
                                        <tr>
                                            ...
                                            <td>{{ number_format($category->blogs_count) }}</td>
                                            <td><span class="status-badge {{ $category->status ? 'active' : 'inactive' }}">{{ $category->status ? 'Active' : 'Inactive' }}</span></td>
                                            <td class="text-end">
                                                <a class="btn btn-sm btn-warning" href="{{ route('admin.blog-categories.edit', $category) }}">Edit</a>
                                                <form class="d-inline" action="{{ route('admin.blog-categories.destroy', $category) }}" method="post" data-delete-confirm>
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    @empty
                                        <tr><td colspan="5" class="text-center text-muted py-5">No blog categories yet.</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
