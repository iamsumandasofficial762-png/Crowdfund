...
<head>
    ...
    <title>Edit Blog Category | Admin</title>
    ...
    <style>
        ...
        .panel { max-width:720px; border:1px solid var(--line); border-radius:18px; background:#fff; box-shadow:0 14px 34px rgba(18,24,39,.07); }
...
<body>
    <div class="admin-layout">
        ...
        <main>
            <header class="topbar"><div class="topbar-inner"><p class="text-muted small mb-1">{{ $blogCategory->slug }}</p><h1 class="h4 fw-bold mb-0">Edit Blog Category</h1></div></header>
            <div class="content">
                @if ($errors->any())
                    <div class="alert alert-danger fw-bold">Please fix the highlighted fields.</div>
                @endif
                <form class="panel p-4" action="{{ route('admin.blog-categories.update', $blogCategory) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label class="form-label fw-bold" for="name">Name</label>
                        <input class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $blogCategory->name) }}" required>
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="form-check form-switch mb-4">
                        <input class="form-check-input" type="checkbox" role="switch" id="status" name="status" value="1" @checked(old('status', $blogCategory->status))>
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        ...
                        <a class="btn btn-soft" href="{{ route('admin.blog-categories.index') }}">Cancel</a>
                    </div>
                </form>
            </div>
        </main>
