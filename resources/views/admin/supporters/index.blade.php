...
<head>
    ...
    <style>
        ...
        .text-clip { max-width:300px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    ...
    <div class="admin-layout">
        <aside class="sidebar">
            ...
            <nav>
                ...
                <a class="nav-link active" href="{{ route('admin.supporters.index') }}"><i class="fa-solid fa-users"></i> Supporters</a>
        ...
        <main>
            <header class="topbar">
                <div class="topbar-inner d-flex align-items-center justify-content-between gap-3">
                    <div>
                        <p class="text-muted small mb-1">Paid donation supporters</p>
                        <h1 class="h4 fw-bold mb-0">Supporters</h1>
                    </div>
                        ...
                        <tbody>
                            @forelse ($supporters as $supporter)
                                <tr>
                                    ...
                                    <td class="fw-bold">{{ $supporter->publicDonorName() }}</td>
                                    <td>{{ $supporter->donor_name ?: '-' }}</td>
                                    <td>{{ $supporter->donor_email ?: '-' }}</td>
                                    <td>{{ $supporter->donor_phone ?: '-' }}</td>
                                    <td>{{ $supporter->is_private ? 'Yes' : 'No' }}</td>
                                    <td class="text-clip">{{ $supporter->fundraiserPost?->title ?? '-' }}</td>
                                    <td>Rs. {{ number_format((float) ($supporter->main_amount ?: $supporter->amount), 0) }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="8" class="text-center text-muted py-5">No supporters yet.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="mt-4">{{ $supporters->links() }}</div>
            </div>
        </main>
    </div>
